...
import { scrapeStudentAssignments } from "../../scrapper/studentAssignmentsScrapper";

export const getStudentAssignments = async (req: Request, res: Response, friendBody?: FriendBody): Promise<void> => {
    try {
        ...
        // Scrape student assignments
        const studentAssignments = await scrapeStudentAssignments(login.cookie);
        ...
        res.status(200).json({
            ...
            lastSynced: new Date().toISOString(),
            errorMessage: error.message || "Invalid assignments data",
        });
    }
}
